$(document).ready(function  () {
    ...
    $.ajax({
    	type:"get",
    	url:"http://192.168.2.152:8080/huajiayi/vmenu/findAll.do",//请求左侧菜单数据
    	datatype:"json",
    	success:function (menu) {
//  		alert("菜单")
    		console.log(menu)
    		var m_data=menu;
    		for (var i=0;i<m_data.length;i++) {//for循环动态创建列表并展示菜单
    			var dd='';
    			dd+='<dd class="menu" data-id="'+m_data[i].id+'">'+m_data[i].name+'</dd>';//创建菜单列表
    			$("#menu").append(dd)
    			
    	    	//将菜单栏数据的id传给后台请求菜单栏对应的视频数据		
    			var id = m_data[i].id;
    			console.log(id)
    			var menus=document.getElementsByClassName("menu");
    			console.log(menus[i])
    			menus[i].onclick=function (vas) {
    				var getid=this.getAttribute("data-id");
					alert(getid)
    				urls="http://192.168.2.152:8080/huajiayi/video/findVideo.do?id="+getid;
    				console.log(urls)
        ...
        pager = $.extend(pager,menu);
        
        //请求数据页面跳转方法
		function goPage(btn){	
	
//          function defaul (){//页面加载时请求的数据            	
//          	urls="http://192.168.2.152:8080/huajiayi/video/findAll.do";
//          }
//      	defaul();//页面加载时调用
//      	request ()//页面加载时调用
//      	
...
//      		urls="http://192.168.2.152:8080/huajiayi/video/descByGrade.do";
//  				request ()
//      	})
//      	
////      	按时间排序
//      	$("#time").click(function () {
//      		urls="http://192.168.2.152:8080/huajiayi/video/descTime.do";
//  				request ()
//      	})
//      	
//      	console.log(urls);
			...
			//                设置当前页码
			               ...
			               function fist (pid) {
			                ...
			                arr.forEach(function(val){  //传参数调用数组中的数据
			                    str+='<div class="movie"><img src="val.ur+imgs" id = "er4"/> <div class="jianjie">'+val.name+'</div> <div class="shichang">'+val.time+'</div> </div>';
			                }); 
			                $('.'+pager.mainBox).html(str);
			                
			                //添加点击事件
			                $(".movie").click(function (vf) {
			                	fist(vf.id)
			                })
					},
					error:function(){
